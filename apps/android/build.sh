...
# ~/android-sdk (see README.md). Output: dist/LabCare-Alerts-v1.0.apk
set -euo pipefail
...
"$BT/apksigner" sign --ks "$KEYSTORE" --ks-key-alias labcare \
    ...
    --out "$OUT/LabCare-Alerts-v1.0.apk" app.aligned.apk

"$BT/apksigner" verify --print-certs "$OUT/LabCare-Alerts-v1.0.apk" | head -3

echo
echo "DONE: $OUT/LabCare-Alerts-v1.0.apk"
